...
// ── Fallback simulado (para cuando la API está sin cuota) ─────────────────────
const FALLBACK = {
  ...
  default: [
    ...
    'Entendido. Aunque ahora mismo opero en modo local, puedo orientarte en temas de desarrollo web, programación y tecnología. ¿Qué parte te genera más dudas?',
...
// Simula streaming letra a letra para el fallback
...
// ── Modelos Gemini a intentar en orden ───────────────────────────────────────
...
app.post('/api/chat', async (req, res) => {
  ...
  // Intenta cada modelo Gemini en orden
  if (genAI) {
    for (const modelName of MODELS) {
      try {
        ...
        if (is429 || is404) {
          console.warn(`⚠️  ${modelName}: ${is429 ? 'cuota agotada' : 'no disponible'}, probando siguiente…`)
          continue
        }
  ...
  // Fallback simulado cuando todos los modelos fallan
  console.log('💬 Usando modo fallback simulado')
...
app.listen(PORT, () => {
  ...
  console.log(`🤖  Modelos: ${MODELS.join(' → ')} → fallback`)
  console.log(`🔑  API key: ${process.env.GEMINI_API_KEY?.slice(0, 8) ?? 'no configurada'}…\n`)
})
